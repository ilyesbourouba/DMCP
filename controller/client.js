...
exports.registerClient = async (req, res, next) => {
    const { username, phone, email, nif, adr, wilaya, password } = req.body;
    try {
        if (!username || !phone || !email || !nif || !adr || !password) {
            return res.status(400).json({ message: "empty_fields" });
        }
        
        // Algerian NIF validation: strictly 15 digits
        if (!/^\d{15}$/.test(nif.trim())) {
            return res.status(400).json({ message: "invalid_nif" });
        }
        ...
        const client = await ClientModel.getClientByPhoneEmail(
            phone.trim(),
            email.trim()
        );
        console.log(client);

        if (client) return res.status(400).json({ message: "user_exists" });

        const hashedPassword = await bcrypt.hashSync(password.trim(), 10);

        const data = await ClientModel.createClient(
            username.trim(),
            phone.trim(),
            email.trim(),
            nif.trim(),
        ...
        return res.status(201).json({ message: "registred" });
    } catch (error) {
        return res.status(500).json({ message: "server_error" });
    }
};
...
exports.updateClient = async (req, res, next) => {
    ...
    try {
        ...
        const clientNew = await ClientModel.getClientByPhoneEmailNotID(
            phone,
            email,
            id
        );
        ...
        if (clientNew) return res.status(400).json({ message: "user_exists" });
...
exports.checkCode = async (req, res, next) => {
    ...
    try {
        console.log("code => ", code);
        if (!code) return res.status(400).json({ message: "empty fields" });
...
exports.resetPassword = async (req, res, next) => {
    ...
    try {
        ...
        const hashedPassword = await bcrypt.hashSync(password.trim(), 10);
        const data = await ClientModel.updateClientPassword(
            code.trim(),
            hashedPassword
        );
...
exports.googleLogin = async (req, res, next) => {
    ...
    try {
        ...
        if (!client) {
            ...
            if (!client.success) {
                console.error(
                    "❌ Failed to create Google client:",
                    client.error
                );
                return res.status(500).json({ message: "server_error" });
            }
